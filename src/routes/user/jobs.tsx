    ...
    <div className="col-xl-4 col-md-6">
      <div className="service-widget servicecontent">
        ...
        <div className="service-content">
          <h3 className="title">
            <Link to={"/job-details"}>{job.name}</Link>
          </h3>
          <p>
            <i className="feather-map-pin"></i>{job.company.location}
          </p>
...
function JobContent({ jobs }: { jobs: Job[] }) {
  return (
    <div className="row">
...
export default function Jobs() {
  const { isLoading, error, data } = useQuery("jobs", () =>
    fetch("http://localhost:8000/job-vacancies").then(
